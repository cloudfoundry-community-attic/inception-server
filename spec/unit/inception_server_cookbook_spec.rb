describe Inception::InceptionServerCookbook do
  ...
  describe "in prepared settings dir" do
    before do
      ...
      expect(subject).to receive(:sh).with("knife solo prepare #{cmd_arguments}")
      subject.prepare

      expect(subject).to receive(:sh).with("knife solo cook #{cmd_arguments}")
      subject.converge
    end
  ...
  describe "after initial converge" do
    it "does not prepare/install chef again" do
      ...
      expect(subject).to receive(:sh).with("knife solo cook #{cmd_arguments}") # just to stub :sh
      subject.prepare
      subject.converge
    end
  end
